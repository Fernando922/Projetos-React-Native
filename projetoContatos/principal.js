...
class Corpo extends Component{
    componentDidMount(){   //quando montado faça
        this.props.getDados()
    }
    ...
    render(){
        let Arr = this.props.listaReducer.myArr.map((a, i) => {
            return (
                    <View key={i}>
                        <TouchableOpacity 
                            onLongPress={() => this.props.remove(i)} 
                            key={i} 
                            style={styles.registro}
                        >  
                            <Image 
                                source={require('./imagem/contato.png')} 
                                style={styles.imagem}
                            />                  
                            <Text style={styles.nome}>{ a.title|| a.nome }</Text>
                            <Text style={styles.telefone}>{ a.title || a.telefone }</Text>
                        </TouchableOpacity>
                    </View>
                    )                            
        }) 
        return (
            ...
            <View style={styles.container}>
                ...
                <TouchableOpacity style={styles.button}>
                    <Text 
                        style={styles.textoBotaoAdd}
                        onPress={() => this.props.handleModal(true) }>+</Text>
                </TouchableOpacity>
...
const mapDispatchToProps = dispatch => bindActionCreators({ adicionar, handleModal, mudaNome, mudaTel, handleConfirmacao, remove, getDados}, dispatch)
